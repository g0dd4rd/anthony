...
def build_app_index():
    ...
    log_and_print(
        f"[SYSTEM] Indexed {len(app_name_map)} app name mappings"
        f" ({gnome_count} org.gnome with priority)"
    )
...
def retrieve_relevant_namespaces(user_input: str, top_k: int = 2) -> tuple:
    ...
    for i, ns in enumerate(relevant_namespaces):
        ...
        log_and_print(
            f"  {i + 1}. {ns} (score: {score:.3f})"
            f" - {len(namespaces[ns]['tools'])} tools{forced_marker}"
        )
...
def build_filtered_tool_schema(relevant_namespaces: list) -> list:
    ...
    filtered_schema = [
        tool for tool in _tool_schema_full if tool["function"]["name"] in relevant_tool_names
    ]

    log_and_print(
        f"[FILTER] Showing {len(filtered_schema)} tools from {len(relevant_namespaces)} namespaces"
    )
