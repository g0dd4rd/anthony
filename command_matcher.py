...
def init(registry, mcp_client, speak_fn, embedding_model, detect_app_fn, check_health_fn=None):
...
def _semantic_match(text, threshold=0.55):
    ...
    if best_score < threshold:
        log_and_print(
            f"[MATCHER] Semantic: no match above threshold (best={best_score:.3f} < {threshold})"
        )
        return None, {}

    entry = _registry.entries[_command_entries_map[best_idx]]
    log_and_print(f"[MATCHER] Semantic match: {entry['name']} (score={best_score:.3f})")
