...
import utils
from app_index import get_friendly_app_name, smart_match_window
...
def run_chain(
    command_messages, filtered_tools, available_tools, direct_mcp_tools, response_start_time
):
    """
    ...
    for chain_step in range(MAX_CHAIN_STEPS):
        ...
        debug_lines.append(
            f"[DEBUG] Available tools: {[t['function']['name'] for t in filtered_tools]}"
        )
        log_and_print("\n".join(debug_lines), level="debug", console=_DEBUG)

        tool_names = [t["function"]["name"] for t in filtered_tools]
        logger.info(
            f"[LLM_REQ] chain_step={chain_step}"
            f" tools={tool_names}"
            f" msg_count={len(command_messages)}"
        )
        max_tokens = 300
        llm_start_time = time.time()
        response = _call_llama_server(
            messages=command_messages, tools=filtered_tools, temperature=0.0, max_tokens=max_tokens
        )
        ...
        if eval_count >= max_tokens and response["message"].get("tool_calls"):
            log_and_print(
                f"[LLM] Output truncated at {max_tokens} tokens, retrying with 600...",
                level="warning",
            )
            response = _call_llama_server(
                messages=command_messages, tools=filtered_tools, temperature=0.0, max_tokens=600
            )
            retry_elapsed = time.time() - llm_start_time - llm_elapsed
            llm_elapsed = time.time() - llm_start_time
            log_and_print(
                f"[TIMING] ⏱️  LLM inference took: {llm_elapsed:.2f}s"
                f" (incl. truncation retry: {retry_elapsed:.2f}s)"
            )
        else:
            log_and_print(f"[TIMING] ⏱️  LLM inference took: {llm_elapsed:.2f}s")

        has_tools = bool(response["message"].get("tool_calls"))
        eval_count = response.get("eval_count", "N/A")
        logger.info(
            f"[LLM_RESP] inference={llm_elapsed:.2f}s"
            f" tokens={eval_count}"
        ...
        if not message.get("tool_calls"):
            ...
            if content:
                ...
                log_and_print(
                    "[COMMAND] ⚠️  No tool call generated. Try rephrasing or switch to chat mode.",
                    level="warning",
                )
                response_time = time.time() - response_start_time
                log_and_print(f"[TIMING] ⏱️  Response time: {response_time:.2f}s")
                _speak(
                    "I'm not sure what command to run. Try rephrasing or say 'switch to chat mode'."
                )
            break
        ...
        for tool_call in message["tool_calls"]:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    log_and_print(
                        f"[COMMAND] ⚠️  LLM returned malformed JSON"
                        f" for {tool_name}:"
                        f" {arguments[:100]}...",
                        level="warning",
                    )
            ...
            if tool_name in direct_mcp_tools:
                ...
                if tool_name == "gnome_search" and "query" in arguments:
                    ...
                    if query.endswith(" website"):
                        ...
                        if not url.startswith("http://") and not url.startswith("https://"):
                            url = f"https://www.{url}"
                        log_and_print(
                            "[GNOME_SEARCH] Detected website marker,"
                            f" opening URL via xdg-open: {url}"
                        )
                        subprocess.run(
                            ["xdg-open", url],
                            check=False,
                        ...
                        log_and_print(
                            f"[GNOME_SEARCH] Detected file marker, using open_file: {filename}"
                        )
                        ...
                        try:
                            ...
                            if match:
                                ...
                                win_title = match.get("title", "")
                                log_and_print(
                                    f"[GNOME_SEARCH] App already running:"
                                    f" {friendly}, focused window"
                                    f" '{win_title}'"
                                )
                            else:
                ...
                if "Error" in result and (
                    "disabled" in result.lower() or "not responding" in result.lower()
                ):
                    log_and_print("[SYSTEM] Tool failed, attempting auto-recovery...")
            ...
            log_and_print(f"\n[OS Feedback]: {result}")
            logger.info(
                f"[TOOL_EXEC] tool={tool_name} args={arguments} result_len={len(str(result))}"
            )
