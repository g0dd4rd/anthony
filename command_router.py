...
import utils
from app_index import get_friendly_app_name, retrieve_relevant_namespaces
from tools.facades import audio_control, system_settings, vision_control, window_control
            ...
            if not result.startswith("Error"):
                ...
                if focused:
                    ...
                    if not shortcut_info.startswith("No shortcuts"):
                        ...
                        log_and_print(
                            f"[ROUTING] Injected shortcuts for focused app '{shortcut_key}'"
                        )
...
def try_short_circuit(
    user_input, user_input_lower, detected_app, auto_focused, retrieval_start_time
):
    """
        ...
        if user_input_lower.startswith(fv):
            ...
            if detected_app and remainder == detected_app and auto_focused:
                ...
                if "No window found" not in result:
                    _speak(result)
                    _log_shortcircuit(
                        f"focused '{remainder}' by window match", retrieval_start_time
                    )
                    return True
            break
    ...
    if "shortcut" in user_input_lower:
        ...
        if shortcut_app:
            ...
            if not shortcut_info.startswith("No shortcuts"):
                ...
                shortcuts = [ln.lstrip("- ").strip() for ln in lines if ln.startswith("- ")]
                ...
                if app_to_tile:
                    try:
                        ...
                        result = window_control(
                            "move_resize", app_to_tile, x=tx, y=ty, width=tw, height=th
                        )
                        _speak(result)
    ...
    if any(p in user_input_lower for p in _mute_tab_phrases + _unmute_tab_phrases):
        ...
        action_word = (
            "Unmuted" if any(p in user_input_lower for p in _unmute_tab_phrases) else "Muted"
        )
    ...
    if user_input_lower in ("mute", "mute the sound", "mute sound", "mute audio"):
        ...
        user_input_lower == p
        for p in ("unmute", "unmute the sound", "unmute sound", "unmute audio")
    ):
        result = audio_control("unmute")
        _speak(result)
        audio_handled = True
    elif any(
        p in user_input_lower
        for p in (
            "volume up",
            "turn up",
            "louder",
    ...
    if "brightness" in user_input_lower or "backlight" in user_input_lower:
        target = (
            "keyboard"
            if any(w in user_input_lower for w in ("keyboard", "kbd", "keys"))
            else "screen"
        )
        level = None
    ...
    # --- Power profile ---
    if (
        "power mode" in user_input_lower
        or "power profile" in user_input_lower
        or "power saver" in user_input_lower
    ):
    ...
    if power_matched:
        ...
        if confirmation and any(
            ...
            for w in ("yes", "yeah", "yep", "sure", "do it", "confirm", "go ahead")
        ):
    ...
    if install_matched and install_query:
        ...
        def _do_install(name, app_id, source):
            verb = "Uninstalling" if is_uninstall else "Installing"
            _speak(f"{verb} {name}. This may take a moment.")
