...
def search_apps(query: str) -> list:
    ...
    source is 'flathub', 'fedora', or 'dnf'."""
    results = []
    seen = set()
    # Flatpak search (preferred source)
    # Try full query first; if no results and multi-word, try first word
    fp_queries = [query]
    if " " in query:
    ...
    for fp_query in fp_queries:
        try:
            ...
            break
    # DNF search (skip names already found in flatpak, cap total at 5)
    try:
        dnf = subprocess.run(
            ["dnf", "search", query],
            capture_output=True, text=True, timeout=15
